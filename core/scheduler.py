...
def scan_tasks():
    ...
    for task in Task.objects.exclude(status='done').select_related('owner'):
      ...
      if new in ('soon','overdue'): notify_once(task.owner,task,new,f'TaskFlow.M: «{task.title}» '+('مهلتش نزدیک است.' if new=='soon' else 'سررسید شده است!'),'task')
    for event in CalendarEvent.objects.filter(notify_telegram=True,starts_at__lte=now+timezone.timedelta(minutes=30),starts_at__gte=now): notify_once(event.created_by,event,'meeting',f'یادآوری TaskFlow.M: {event.title}','event')
